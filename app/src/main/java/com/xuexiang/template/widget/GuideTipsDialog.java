package com.xuexiang.template.widget;
...
import com.xuexiang.template.utils.MMKVUtils;
...
public class GuideTipsDialog extends BaseDialog implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private static final String KEY_IS_IGNORE_TIPS = "com.xuexiang.template.widget.key_is_ignore_tips_";
    ...
    public static void showTipsForce(Context context) {
        Type listType = new TypeToken<List<TipInfo>>() {}.getType();
        List<TipInfo> tipInfoList = JsonUtils.loadListFromRaw(context, R.raw.tips, listType);
        if (tipInfoList != null && tipInfoList.size() > 0) {
